...
ClassImp(ReCenter)
    ...
    StRefMultCorr* ReCenter::mRefMultCorr = NULL;
    //-----------------------------------------------------------------------------
    ReCenter::ReCenter(const char* name, StPicoDstMaker *picoMaker, char* jobid, std::string configFileName)
: StMaker(name)
{
    ...
    mOutPut_Rec=Form("%s_recenter.root",jobid);
}
...
Int_t ReCenter::Init() 
{
    ...
    //TFile *mRecenteringInputFile = TFile::Open("/star/data01/pwg/dchen/Ana/19p6GeV/eventplane/19p6gev_recenterpar.root"); // more test
...
//----------------------------------------------------------------------------- 
void ReCenter::Clear(Option_t *opt) {
}
...
Int_t ReCenter::Make() 
{
	...
	{ // recenter the full event plane, shift parameter
	   mEpProManager->FillPsiRecFull_tpc(theEvent_tpc->GetEventPsi(2));
           ...
           {
               // first order shift parameter
	       mEpProManager->FillFullEpShiftpar_tpc(iorder, cent9, runIndex, theEvent_tpc->GetEventPsi(1));
	   }
        } // recenter the full event plane, shift parameter
...
int ReCenter::GetRunIndex(int runID)
{
    int runIndex=-999;
...
/*int ReCenter::Centrality(int gRefMult )
{
    int centrality;
